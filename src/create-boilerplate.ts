...
const main = async () => {
  ...
  const oises = emptyObject(apiDataTemplate.ois,["oisFormat","version"],["paths","fixedOperationParameters","reservedParameters","parameters"]) as Oises;
  ...
  const templateBoilerPlate =  { [response.apiName + "-" + "tempate01.json"] : templates[Object.keys(templates)[0]] } as Templates;

  // Create the boilerplate beacon
  const beacons = emptyObject(apiDataTemplate.beacons,["updateConditionPercentage","active","walletType","deviationFactorThreshold","ttlMinutes"],["decodedParameters"]) as Beacons
